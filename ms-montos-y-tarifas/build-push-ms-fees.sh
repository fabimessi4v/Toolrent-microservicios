...
echo -e "${BLUE}⚡ BUILD & PUSH - MS-FEES${NC}"

# Verificar directorio
if [ ! -f "pom.xml" ] && [ ! -f "build.gradle" ]; then
    echo -e "${RED}❌ Ejecuta desde el directorio ms-fees/${NC}"
    exit 1
fi
...
echo -e "  kubectl set image deployment/ms-fees ms-fees=${IMAGE_TAG}"
echo -e "  kubectl rollout restart deployment/ms-fees"
