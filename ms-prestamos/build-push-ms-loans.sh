...
DOCKER_REPO="ms-prestamos"   # Imagen = fabimessidev/ms-prestamos

echo -e "${BLUE}⚡ BUILD & PUSH - MS-LOANS${NC}"

# Verificar directorio
if [ ! -f "pom.xml" ] && [ ! -f "build.gradle" ]; then
    echo -e "${RED}❌ Ejecuta desde el directorio ms-loans/${NC}"
    exit 1
fi
...
echo -e "  kubectl set image deployment/ms-loans ms-loans=${IMAGE_TAG}"
echo -e "  kubectl rollout restart deployment/ms-loans"
echo ""
