...
# ============================================
# TOOLRENT MS-CUSTOMERS - BUILD & PUSH
# ============================================
...
DOCKER_REPO="ms-customers"

echo -e "${BLUE}⚡ BUILD & PUSH - MS-CUSTOMERS${NC}"

# Verificar directorio
if [ ! -f "pom.xml" ] && [ ! -f "build.gradle" ]; then
    echo -e "${RED}❌ Ejecuta desde el directorio ms-customers/${NC}"
    exit 1
fi
...
if [ -f "pom.xml" ]; then
    ...
    ./gradlew clean bootJar -x test || { echo -e "${RED}❌ Build falló${NC}"; exit 1; }
    echo -e "${GREEN}   ✅ Build exitoso${NC}"
fi

# 2. Docker build
echo -e "${BLUE}2/4 Creando imagen Docker...${NC}"
docker build -t ${IMAGE_TAG} -t ${IMAGE_LATEST} . || { echo -e "${RED}❌ Docker build falló${NC}"; exit 1; }
...
echo -e "  kubectl set image deployment/ms-customers ms-customers=${IMAGE_TAG}"
echo -e "  kubectl rollout restart deployment/ms-customers"
