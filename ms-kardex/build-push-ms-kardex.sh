...
DOCKER_REPO="ms-kardex"

echo -e "${BLUE}⚡ BUILD & PUSH - MS-KARDEX${NC}"

# Verificar directorio
if [ ! -f "pom.xml" ] && [ ! -f "build.gradle" ]; then
    echo -e "${RED}❌ Ejecuta desde el directorio ms-kardex/${NC}"
    exit 1
fi
...
echo -e "  kubectl set image deployment/ms-kardex ms-kardex=${IMAGE_TAG}"
echo -e "  kubectl rollout restart deployment/ms-kardex"
