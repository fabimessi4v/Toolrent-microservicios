...
# ============================================
# TOOLRENT MS-INVENTORY - BUILD & PUSH
# ============================================
...
DOCKER_REPO="ms-inventory"

echo -e "${BLUE}⚡ BUILD & PUSH - MS-INVENTORY${NC}"

# Verificar directorio
if [ ! -f "pom.xml" ] && [ ! -f "build.gradle" ]; then
    echo -e "${RED}❌ Ejecuta desde el directorio ms-inventory/${NC}"
    exit 1
fi
...
echo -e "  kubectl set image deployment/ms-inventory ms-inventory=${IMAGE_TAG}"
echo -e "  kubectl rollout restart deployment/ms-inventory"
