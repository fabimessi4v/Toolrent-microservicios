...
DOCKER_REPO="ms-frontend"

echo -e "${BLUE}⚡ BUILD & PUSH - Frontend${NC}"

# Verificar directorio
if [ ! -f "package.json" ]; then
    echo -e "${RED}❌ Ejecuta desde el directorio frontend/${NC}"
    exit 1
fi
...
# 1. Build de la app
echo -e "${BLUE}1/4 Building aplicación...${NC}"
npm run build || { echo -e "${RED}❌ Build falló${NC}"; exit 1; }
...
echo -e "  kubectl set image deployment/frontend frontend=${IMAGE_TAG}"
